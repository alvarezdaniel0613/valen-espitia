...
export function PhotoHero(){
 ...
 return <section id="inicio" className="photo-hero" aria-label="Valen Espitia en vivo">
  <Carousel opts={{loop:true}} setApi={setApi} className="photo-carousel" aria-label="Fotografías de Valen en vivo">
   <CarouselContent className="photo-track">{photos.map((photo,i)=><CarouselItem className="photo-slide" key={photo.id} aria-label={`Foto ${i+1} de 4`}><picture><source media="(max-width: 600px)" srcSet={`${import.meta.env.BASE_URL}live-${photo.id}-portrait-900.webp 900w, ${import.meta.env.BASE_URL}live-${photo.id}-portrait-1350.webp 1350w`} sizes="max(100vw, 56.25svh)"/><img src={`${import.meta.env.BASE_URL}live-${photo.id}.webp`} srcSet={`${import.meta.env.BASE_URL}live-${photo.id}.webp ${photo.id==='0820'||photo.id==='1231'?2560:1707}w, ${import.meta.env.BASE_URL}live-${photo.id}-3200.webp 3200w`} sizes={photo.id==='0820'||photo.id==='1231'?'max(100vw, 150svh)':'max(100vw, 67svh)'} alt={photo.alt} style={{objectPosition:photo.position}} loading={i===0?'eager':'lazy'} fetchPriority={i===0?'high':'auto'} width={photo.id==='0820'||photo.id==='1231'?2560:1707} height={photo.id==='0820'||photo.id==='1231'?1709:2560}/></picture></CarouselItem>)}</CarouselContent>
